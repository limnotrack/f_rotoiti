...
decode_var_ref <- function(var_ref, variable_ref = NULL) {
  lapply(var_ref, \(v) {
    ...
    data.frame(
      var_abbr = variable,
      reference = reference,
      z_relative = value_cm / 100  # Convert cm back to meters
    )
  }) |> 
    dplyr::bind_rows()
}
  ...
  name <- tryCatch({
    decode_var_ref(var_ref = var_ref, variable_ref = variable_ref) |> 
      dplyr::mutate(
        ...
        var_name = paste0(var_abbr, " at ", z_relative, "m ", pos)
      ) |> 
      dplyr::pull(var_name)
  }, error= function(e) {
...
plot_var_ts_qc <- function(data, var_ref_id, #variable_ref,
  ...
  device_pos <- decode_var_ref(sel_vars) |> 
    ...
    dplyr::arrange(dplyr::desc(z_relative))
...
plot_var_doy <- function(data, var_ref_id, days_offset = 184, FUN = mean, 
  ...
  z <- unique(df$z_relative)
  ord_z <- sort(z)
  
  df <- df |> 
    dplyr::mutate(z_relative = factor(z_relative, levels = ord_z)) |> 
...
# Extract device metadata from the data
extract_device_metadata <- function(data) {
  
  devices <- unique(data$device_id)
  
  site_devices <- lapply(devices, function(device) {
    data |> 
      dplyr::filter(device_id == device) |> 
      dplyr::mutate(date_from = min(datetime),
                    date_to = max(datetime)) |>
      dplyr::select(site, device_id, date_from, date_to) |> 
      dplyr::distinct()
  }) |> 
    dplyr::bind_rows() |> 
    dplyr::arrange(site, device_id, date_from)
  ...
  device_position <- lapply(devices, function(device) {
    data |> 
      dplyr::filter(device_id == device) |> 
      dplyr::distinct(site, device_id, var_ref_id     ) |> 
      dplyr::mutate(
        z_relative = decode_depth(var_ref_id),
        reference = decode_reference(var_ref_id)
      ) |>
      dplyr::select(site, device_id, reference, z_relative)
  }) |> 
    dplyr::bind_rows()
  
  return(list(site_devices = site_devices, 
